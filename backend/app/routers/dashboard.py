...
# ✅ Removed internal prefix to avoid /dashboard/dashboard duplication
router = APIRouter(
    tags=["Dashboard"],
...
def my_day_dashboard(
    ...
    """
    Returns key metrics for 'My Day' — today's leads, tasks, reminders,
    and nearby leads within a given radius.
    """
    ...
    new_leads = (
        ...
        .filter(
            ...
            Lead.created_at <= end_today,
        )
        .count()
    )

    # Tasks today
    tasks_today_q = db.query(Task).filter(
    ...
    yesterday_count = (
        ...
        .filter(
            Task.company_id == current_user.company_id,
            Task.due_date >= start_yesterday,
            Task.due_date <= end_yesterday,
        )
        .count()
    )
