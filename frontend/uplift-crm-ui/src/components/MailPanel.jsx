...
import client from "@/api/client"; // ✅ Global axios instance (auto handles token)

/* ======================================
   Helpers & Config
====================================== */
const API_BASE = import.meta.env.VITE_API_BASE_URL; // ✅ matches your .env
...
export default function MailPanel({ lead, onBack }) {
  ...
  const axiosClient = useMemo(() => {
    const c = client;
    if (userEmail) c.defaults.headers["X-User-Email"] = userEmail;
    return c;
  }, [userEmail]);
  ...
  async function loadMessages() {
    ...
    try {
      ...
      const { data } = await axiosClient.get(
        `/integrations/gmail/messages/${encodeURIComponent(leadEmail)}`
      );
  ...
  async function sendMail() {
    if (!draft.trim()) return alert("Message cannot be empty.");
    try {
      // ✅ correct endpoint
      await axiosClient.post(`/integrations/gmail/reply`, {
        to: leadEmail,
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="border-t border-white/10 p-2">
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={sendMail}
            className="px-3 py-2 bg-[#00BFFF] text-[#0B1222] rounded-lg hover:opacity-90 flex items-center gap-2"
          >
            <Send size={16} /> Send
          </button>
        </div>
